...
def parse_arguments():
    ...
    parser.add_argument(
        "--normalized-tandem-pas-expression",
        dest="normalized_tandem_pas_expression",
        required=True,
        help="Path for the TSV file with normalized tandem pas expression values.",
    )
    return parser
...
def main():
    ...
    tandem_pas_expression.replace(to_replace=np.nan, value=-1.0).to_csv(
        options.normalized_tandem_pas_expression,
        sep="\t",
        index=False,
        float_format="%.6f",
    )
